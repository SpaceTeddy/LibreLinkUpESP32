...
uint8_t LIBRELINKUP::begin(uint8_t use_cert) {
    ...
    // use_cert == 3: caller sets the certificate via setCACertBundle() after begin()

    return 1;
}

/**
 * @brief Configures the internal WiFiClientSecure with an x509 certificate bundle.
 *
 * Arduino core 3.x needs the bundle length as well -- the underlying client no
 * longer derives it from the bundle header. Callers embedding the bundle via
 * platformio's board_build.embed_files get it from the linker symbols:
 * `x509_crt_bundle_end - x509_crt_bundle_start`.
 *
 * @param bundle Pointer to a bundle generated by ESP-IDF's gen_crt_bundle.py.
 * @param size   Bundle length in bytes (ignored on Arduino core 2.x).
 */
void LIBRELINKUP::setCACertBundle(const uint8_t* bundle, size_t size) {
#if defined(ESP_ARDUINO_VERSION) && ESP_ARDUINO_VERSION >= ESP_ARDUINO_VERSION_VAL(3, 0, 0)
    secure_client_.setCACertBundle(bundle, size);
#else
    (void)size;
    secure_client_.setCACertBundle(bundle);
#endif
}
